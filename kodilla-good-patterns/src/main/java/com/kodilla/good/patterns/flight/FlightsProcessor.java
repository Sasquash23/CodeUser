...
public class FlightsProcessor {
    ...
    public List<ConnectingFlight> getFlightsThrough(Airport through) {
        List<ConnectingFlight> flightsThrough = new ArrayList<>();
        ...
        for (int i = 0; i < tempFrom.size(); i++) {
            for (int j = 0; j < tempTo.size(); j++) {
                if (tempFrom.get(i) != tempTo.get(j)) {
                    flightsThrough.add(new ConnectingFlight(tempFrom.get(i), through, tempTo.get(j)));
                }
            }
        }
    ...
    public List<Flight> findFlight(Airport from, Airport to) {
        ...
        if (resultFindingFlights.size() == 0) {
            ...
            for (Airport airport: throughAirports) {
                ConnectingFlight flyThrough = getFlightsThrough(airport).get(
                        getFlightsThrough(airport).indexOf(new ConnectingFlight(from, airport ,to)));
